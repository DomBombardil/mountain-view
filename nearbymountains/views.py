...
@require_GET
def mountain_photo_api(request):
    name = request.GET.get("name", "").strip()

    if not name:
        return JsonResponse(
            {"error": "Mountain name is required."},
            status=400
        )
    ...
    try:
        ...
        return JsonResponse(
            {"error": "Could not retrieve mountain photo."},
            status=502
        )
    ...
    image_info = first_page.get("imageinfo", [{}])[0]
    ...
    return JsonResponse({
        ...
        "artist": metadata.get("Artist", {}).get("vaulue"),
